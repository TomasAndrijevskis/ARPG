...
void AFireStorm::HandleOverlap(AActor* OtherActor, bool bIsOverlapping)
{
	...
	if (Cast<AEnemyCharacter>(OtherActor))
	{
		...
		OverlappedActor->FireStatusEffectManager->SetParams(BurnDamage, BurnDuration, BurnRate, bIsOverlapping);
		OverlappedActor->FireStatusEffectManager->HandleEffect(bIsOverlapping);
	}
}


void AFireStorm::SetParams(const float NewDuration, const float NewDamage, const float NewBurnRate){BurnDuration = NewDuration;BurnDamage = NewDamage;BurnRate = NewBurnRate;}